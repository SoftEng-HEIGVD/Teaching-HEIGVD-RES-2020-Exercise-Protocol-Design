...
public class PresenceServer implements Runnable {
	...
	public void run() {
		try {
			LOG.log(Level.INFO, "Starting Presence Server on port {0}", serverPort);
	...
	class Worker implements Runnable {
		...
		public void run() {
			...
			PresenceServer.this.notifyConnectedWorkers("  OPEN THE CALCULATOR WITH CALC");
				...
				while (connected && ((commandLine = in.readLine()) != null)) {
					...
					switch (tokens[0].toUpperCase()) {
						...
						case (Protocol.CMD_KILL):
							...
							shutdown();
							break;
						case (Protocol.CMD_CALC):
							sendNotification("Welcome to the the calculator press h for help or q to quit");
							try {
								String calcCommand;
								boolean inCalc = true;
								int res;
								while (inCalc && ((calcCommand = in.readLine()) != null)) {
									String[] cmd = calcCommand.split(" ");
									switch (cmd[0].toUpperCase()) {
										case ("H"):
										case ("HELP"):
											sendNotification("HELP :");
											sendNotification("		These are the calculator commands available :");
											sendNotification("		H or help to get help");
											sendNotification("		Q or quit to quit");
											sendNotification("		Operations: ADD,SUB,MUL,DIV");
											sendNotification("		Operations format : OP n1 n2 (n1 and n2 should be numbers)");
											break;

										case ("Q"):
										case ("QUIT"):
											sendNotification("Quitting the calculator");
											inCalc = false;
											break;

										case ("ADD"):
											res = Integer.parseInt(cmd[1]) + Integer.parseInt(cmd[2]);
											sendNotification(cmd[1] + " + " + cmd[2] + " = " + res);
											break;
										case ("SUB"):
											res = Integer.parseInt(cmd[1]) - Integer.parseInt(cmd[2]);
											sendNotification(cmd[1] + " - " + cmd[2] + " = " + res);
										...
										default:
											sendNotification("Unknown command type h for help");
											break;
									}
								}
							} catch(Exception ex) {
								sendNotification("Unknown command / Wrong format, Calculator crashed !");
							}
							break;

						default:
							sendNotification("What? I only understand HELLO, SAY, WHO, CALC, BYE and KILL commands");
					}
				}
			} catch (IOException ex) {
				LOG.log(Level.SEVERE, ex.getMessage(), ex);
